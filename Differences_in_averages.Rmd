...
averaged_by_site_SSMH <- SUMMERGRND23_ALLDEPTHS %>% filter(fullname == "SSMH-BASE" | fullname == "SSMH-SA" | fullname == "SSMH-SB"| fullname == "SSMH-SC" | fullname == "SSMH-SD") %>% aggregate(value~date+depth+val, mean, na.rm=FALSE)
averaged_by_site_SSMH$station <- c("SSMH")
...
averaged_by_station5$fullname <- c("BEO")
SUMM3_STATIONS_AVG<- rbind(averaged_by_station, averaged_by_station2, averaged_by_station3,averaged_by_station4,averaged_by_station5)
```


STATS: average daily difference between SPECIFIC STATIONS for Xcm depth (define in x <- )
```{r}
wide <- SUMM3_STATIONS_AVG %>%
    pivot_wider(names_from = fullname, values_from = value)
