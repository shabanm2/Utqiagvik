...
df <- read.csv("~/Desktop/UVA/RESEARCH/Barrow/GAP_FILLED_CSV_DOWNLOADS/Sep/all_years/all_var_combined_22_25_BEO_COMP.csv")
...
ggplot(scores, aes(x=PC1/sd[1], y=PC2/sd[2]))+ #sets up the plot
  ...
  ylab("PC-2 (22%)") + xlab("PC-1 (46%)") + ggtitle("Summer 2023 Residential and Hospital")
...
fall <- df %>%
  subset(Date >="2022-10-01" & Date <= "2022-10-31") %>%
  subset(!site == "BEO") %>%
  subset(sensor_name %in% c("TNHA-SA", "TNHA-SC", "SSMH-SA", "SSMH-SB" )) %>%
  na.omit() %>%
  unique() %>%
...
ggplot(scores, aes(x=PC1/sd[1], y=PC2/sd[2]))+ #sets up the plot
  ...
  theme(axis.text.x = element_blank(),  # remove x-axis text
        ...
        legend.title = element_text(size=14),
        legend.position = "right") +
  ylab("PC-2 (25%)") + xlab("PC-1 (49%)") + ggtitle("Fall 2022 Residential and Hospital")
