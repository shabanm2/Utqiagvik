---
title: "Double Y Axis"
author: "Elizabeth Van Metre, Edited by Mirella Shaban as of 2024"
date: "2024-01-24"
...
windspeed = read.csv(paste0(filepath, windspeed))
winddir = read.csv(paste0(filepath, winddir))
```
...
tnha = full_join(tnha, tnha_solar, by=c("date", "station", "sensor")) %>% select(c(1:6,8))
...
Fixing Gaps
```{r}
d = rep(seq(as.Date(tnha2$date[1]), as.Date(tnha2$date[nrow(tnha2)]), by="day"),7)
val = rep(1, length(d))
df = data.frame(d, val)
colnames(df) = c("date","value")
df2 = left_join(df, tnha2, by=c("date"))

#fix this, diff. lgenths
```
...
ggplot(tnha2, aes(x=date)) +
  ...
  geom_line( aes(y=solar, color="#D95F02"), linewidth=0.4) + #make dashed/thicker
