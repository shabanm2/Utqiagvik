---
title: "Differences_in_averages_VWC"
...
############################################################
THIS SCRIPT GIVES YOU THE SEASONAL AVERAGES *(VWC)*
...
VWC: VWC_DLYAVG_COMB_22_23
#need to split this now into date range?


Split into seasons for station vs station analysis (no need to run, should be last chunk in the load and clean script)
```{r}
...
```{r}
averaged_by_station <- VWC_Summer23_Daily %>% filter(fullname == "TNHA-SA") %>% aggregate(value~date+depth, mean)
averaged_by_station$fullname <- c("TNHA-SA")
averaged_by_station2 <- VWC_Summer23_Daily %>% filter(fullname == "TNHA-SC") %>% aggregate(value~date+depth, mean)
averaged_by_station2$fullname <- c("TNHA-SC")
averaged_by_station3 <- VWC_Summer23_Daily %>% filter(fullname == "SSMH-SA") %>% aggregate(value~date+depth, mean)
averaged_by_station3$fullname <- c("SSMH-SA")
averaged_by_station4 <- VWC_Summer23_Daily %>% filter(fullname == "SSMH-SB") %>% aggregate(value~date+depth, mean)
averaged_by_station4$fullname <- c("SSMH-SB")
averaged_by_station5 <- VWC_Summer23_Daily %>% filter(fullname == "BEO-B05" | fullname == "BEO-B06") %>% aggregate(value~date+depth, mean)
averaged_by_station5$fullname <- c("BEO")
# averaged_by_station6 <- VWC_Summer23_Daily %>% filter(fullname == "BEO-B07" | fullname == "BEO-B08") %>% aggregate(value~date+depth, mean)
# averaged_by_station6$fullname <- c("BEO=NEW")
#BEO-B08/07 DOESNT SHOW IN DATA UNTIL FALL...
sum23_STATIONS_AVG_VWC<- rbind(averaged_by_station, averaged_by_station2, averaged_by_station3,averaged_by_station4,averaged_by_station5)
```


STATS: average daily difference between SPECIFIC STATIONS for Xcm depth (define in x <- )
```{r}
wide <- sum23_STATIONS_AVG_VWC %>%
