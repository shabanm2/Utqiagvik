...
```
```{r}
#repeat measures anova for ground temperatures
data <- averaged_BEO_TNHA_SSMH
time <- averaged_BEO_TNHA_SSMH$day
value <- averaged_BEO_TNHA_SSMH$value
station <- averaged_BEO_TNHA_SSMH$station

model <- aov(value~factor(time) + Error(factor(station)), data = data)
summary(model)
