...
averaged_by_site_BEO <- SUMMERGRND22_ALLDEPTHS %>% filter(fullname == "BEO-B06") %>% aggregate(value~date+depth, mean)
...
averaged_by_site_BEO <- FALL22_ALLDEPTHS %>% filter(fullname == "BEO-B06") %>% aggregate(value~date+depth, mean)
averaged_by_site_BEO$station <- c("BEO") 
...
Merge summer 22 + summer 23 data-sets for plotting
```{r}
...
averaged_by_site_BEO <- SUMMERGRND23_ALLDEPTHS %>% filter(fullname == "BEO-B06") %>% aggregate(value~date+depth, mean)
averaged_by_site_BEO$station <- c("BEO") 
