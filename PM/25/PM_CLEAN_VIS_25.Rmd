...
filepath <- "https://raw.githubusercontent.com/shabanm2/Utqiagvik/main/PM/25/25_Data/"
file1 <- "Hourly_June1_17.csv"
file2 <- "July1to30.csv"
file3 <- "SUM_PM_618_712.csv"
file4 <- "july30tosep1.csv"

a <- read_csv(paste0(filepath, file1))
b <- read_csv(paste0(filepath, file2))  # Changed to read_csv
c <- read_csv(paste0(filepath, file3))  # Changed to read_csv
d <- read_csv(paste0(filepath, file4))  # Changed to read_csv

df_unique <- bind_rows(a, b, c, d)

df_unique$Time <- as.POSIXct(df_unique$Time, format = "%m-%d-%Y %H:%M")
...
```{r}
#only run if getting off of desktop
