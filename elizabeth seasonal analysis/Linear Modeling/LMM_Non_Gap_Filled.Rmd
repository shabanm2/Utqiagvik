...
-   4/9/2025: copy script from LMM_NvS.Rmd and update to use old, non-gap-filled data
-   5/30/2025: change filepath to use daily csv exported by Daily_Avg_All_Seasons_2025.R


## Directory Location
```{r}
# location of the current working directory, ending with a '/'
# this should be the root folder where you cloned the github repository
# (it should be called "Utqiagvik" unless you renamed it)
rootpath = "~/Desktop/Arctic/Utqiagvik/"
```
...
This code is for reading data directly from the github, if the local files are not working.
...
combined = full_join(grndtmp, airtmp, by=c("date", "station", "sensor")) %>% select(c(2:5,6,9))
colnames(combined)[2:6] = c("station","sensor", "grounddepth", "groundtemp", "airtemp")
combined = full_join(combined, wind, by=c("date", "station", "sensor")) %>% select(c(1:6,9))
colnames(combined)[7] = c("windspeed")
combined = full_join(combined, solar, by=c("date", "station", "sensor")) %>% select(c(1:7,10))
colnames(combined)[8] = c("solar")

combined <- combined %>% mutate(depth = case_when(grounddepth == 7 ~ 1,
          grounddepth == 8 ~ 1,
          grounddepth == 9 ~ 2,
          grounddepth == 10 ~ 3,
          grounddepth == 11 ~ 3,
          ...
          grounddepth == 15 ~ 6,
          grounddepth == 16 ~ 6,
          grounddepth == 17 ~ 6)) %>% filter(grounddepth != 1)

combined = full_join(combined, vwc, by=c("date", "station", "sensor", "depth")) %>% select(c(1:9,11))
colnames(combined)[c(2:3,9:10)] = c("site", "fullname", "vwcdepth", "vwc")
df <- combined
```

```{r}
filepath = paste0(rootpath, "Analysis_Ready_Data/Daily/")
...
daily <- daily %>% filter(fullname == "TNHA-SA" | fullname == "TNHA-SC" | fullname == "SSMH-SB" | fullname == "SSMH-SA" | fullname == "BEO-BASE") %>% select(-c(winddirection)) %>% mutate(aspect = case_when(fullname == "TNHA-SC" | fullname == "SSMH-SB" ~ "North", fullname == "TNHA-SA" | fullname == "SSMH-SA" ~ "South", .default = "N/A")) %>% filter(grounddepth == "10cm") %>% filter(Date >= "2022-06-19") %>% na.omit()
df <- daily
```
...
```{r}
nvs <- df %>% filter(fullname == "TNHA-SA" | fullname == "TNHA-SC" | fullname == "SSMH-SB" | fullname == "SSMH-SA") %>% mutate(aspect = case_when(fullname == "TNHA-SC" | fullname == "SSMH-SB" ~ "North", .default = "South")) %>% na.omit() %>% filter(windspeed >= 0)
```
...
df_10cm_summer_2022 <- get_dates(nvs, "10cm", "2022-06-19", "2022-09-01")
```
...
fit_lmm <- function(df) {
  ...
  if (length(levels(as.factor(df$site))) > 1) {
    lmm <- lmer(groundtemp ~ airtemp + vwc + solar + windspeed + aspect + (1|site), data = df)
  } else {
    # lmm <- lmer(groundtemp ~ airtemp + vwc + solar + windspeed + aspect, data = df)
    return(NA)
  }
...
fit_lmm_correlated <- function(df) {
  # need to check if there is enough data in the df
  # check if there are any observations in df
  if (nrow(df) > 0) {
...
if (is.na(summer2022_lmm)) {
  print("Error with LMM - either no valid data or not enough valid levels of 'site'")
} else {
