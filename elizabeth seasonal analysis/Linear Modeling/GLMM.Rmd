...
df <- read.csv(paste0(filepath, "all_sites_daily_gap_filled.csv"))
df <- df %>% select(-X)
...
**Random effects** tells you how much variance you find among levels of your grouping factor(s) as well as the residual variance. The residual variance is the variation explained by the fixed effects.

**Fixed effects** are similar to a linear model output with slope and intercept as well as their respective errors.
...
We need to make sure that our response variable is normally distributed. If it is not, then we will have to standardize groundtemp.
...
It is good practice to scale our explanatory variables so that we can effectively compare the effects. Scaling keeps the distribution of the variable the same shape but it makes the mean value zero and the standard deviation 1.
...
BEO
CONTROL: BEO-BASE | BEO-B05 | BEO-B06 | BEO-B07 | BEO-BO8
...
summary(mixed.lmer)
```
The variance for station is very low (0.04664) especially compared to the residual (15.27). This means that the aspect of our sensor does not account for much variation in ground temp and instead airtemp accounts for the majority of the variation.

We can calculate the percentage of the random effect's contribution to variance:
