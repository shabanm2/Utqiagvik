...
Load Data
```{r}
grndtmp = read.csv(url((paste0(filepath, gtfile)))
airtmp = read.csv(url(paste0(filepath, airfile)))
vwc = read.csv(url(paste0(filepath, vwcfile)))
solar = read.csv(url(paste0(filepath, solfile)))
wind = read.csv(url(paste0(filepath, windspeed)))
```
...
## PCA
```{r}
tnha_pca <- prcomp(tnha[,4:7], center=TRUE, scale.=TRUE)

#take out variables
sd <- tnha_pca$sdev
loadings <- tnha_pca$rotation
rownames(loadings) <- colnames(tnha[4:7])
scores <- tnha_pca$x
...
abline(h=1/ncol(tnha[4:7])*100, col="red")
```

### Loadings
```{r}
loadings
sqrt(1/ncol(tnha[4:7])) # cutoff for "important" loadings
```
...
```{r}
ggplot(cluster_sensors, aes(x = PC1, y = PC2, shape = cluster)) +
  geom_point(aes(color = tnha.sensor), size = 2) +
  scale_shape_manual(values = c(16, 17, 18)) +  # Set shapes manually
  theme_minimal() +
  labs(title = "Biplot with Custom Point Symbols",
       x = "PC1",
       y = "PC2")#+ 
  #geom_segment(data = as.data.frame(eigenvectors), aes(x = 0, y = 0, xend = PC1, yend = PC2),
                # arrow = arrow(type = "closed", length = unit(0.2, "inches")),
                 #size = 0.5, color = "black") +
  #geom_text(data = as.data.frame(eigenvectors), aes(x = PC1, y = PC2, label = variable), vjust = -1, hjust = -1)
