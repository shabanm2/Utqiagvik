...
calc_pca <- function(pca_df){
  pca <<- prcomp(pca_df[,7:(ncol(pca_df)-1)], center=TRUE, scale.=TRUE)
...
$ \vec{v} = \lambda \vec{v} $ where $\vec{v}$ is an eigenvector and $\lambda$ is our eigenvalue scalar.

If we set our equation equal to zero such that \$ A\vec{v} - \lambda \vec{v} = 0\$ and solve for $\lambda$.
