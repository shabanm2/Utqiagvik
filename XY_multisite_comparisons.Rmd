...
```{r}
# importing ground temp data for 6-19 through 9-20
grndtmp619_920=read.csv(paste(filepath,"Summer_grndtmp_2023_04_06_12_21_30_UTC_1.csv", sep=""))
...
Ground Moisture for all sites data prepping
```{r}
mst619_920=read.csv(paste(filepath, "Summer_VWC_2023_04_06_12_33_54_UTC_1.csv", sep=""))
...
Solar radiation for all sites data prepping
```{r}
solarrad619_920 = read.csv(paste(filepath,"Summer_solar_2023_04_06_13_16_22_UTC_1.csv", sep= ""))
...
XY_grnd_air= averages %>% filter(station == "TNHA" | station == "BEO", !sensor == "TNHA-BASE", !sensor == "TNHA-SD", !sensor == "TNHA-SB") 
  ...
  geom_point()+
    guides(color = guide_legend(override.aes = list(size=10)))+
  theme_bw() + 
    labs(title= "TNHA-SA(S), TNHA-SC(N) vs. BEO-B06 @ 10cm depth, 6/19/22-9/19/22", x = "Air Temperature", y="Ground Temperature (°C) @ 10cm",color="Station") + 
  theme(axis.text.y = element_text(size = 15)) +
  theme(axis.text.x=element_text(angle = 45, hjust = 1.0, size = 15), 
        axis.line = element_line(colour = "black", size =0.5, 
                                 linetype = "solid")) +
...
XY_grnd_air= averages %>% filter(station == "TNHA" | station == "BEO") 
  ...
  geom_point()+
    guides(color = guide_legend(override.aes = list(size=10)))+
  theme_bw() + 
    labs(title= "TNHA stations vs. BEO @ 10cm depth, 6/19/22-9/19/22", x = "Air Temperature (°C)", y="Ground Temperature (°C)",color="Station") + 
  theme(axis.text.y = element_text(size = 15)) +
  theme(axis.text.x=element_text(angle = 45, hjust = 1.0, size = 15), 
        axis.line = element_line(colour = "black", size =0.5, 
                                 linetype = "solid")) +
...
GROUND TEMP VS GROUND TEMP

```{r}
XY_grnd_vs_grnd= averages %>% filter(station == "TNHA" | station == "BEO", !sensor == "TNHA-SA", !sensor == "TNHA-SB", !sensor == "TNHA-SC",!sensor =="TNHA-SD", !sensor =="TNHA-BASE", !sensor == "BEO-BASE") 

  ggplot(data = XY_grnd_vs_grnd, aes(x=grndtmp, y = grndtmp, color=sensor)) +
  geom_point()+
    guides(color = guide_legend(override.aes = list(size=10)))+
  theme_bw() + 
    labs(title= "BEO-B05 vs. BEO-B06 @ 10cm depth, 6/19/22-9/19/22", x = "Ground Temperature (°C) @ 10cm", y="Ground Temperature (°C) @ 10cm",color="Station") + 
  theme(axis.text.y = element_text(size = 15)) +
  theme(axis.text.x=element_text(angle = 45, hjust = 1.0, size = 15), 
        axis.line = element_line(colour = "black", size =0.5, 
                                 linetype = "solid")) +
  ...
  theme(legend.title = element_text(size = 15),
        ...
        legend.key.height = unit(1,'cm')) +
    geom_smooth(method=lm, se=TRUE) + #add a line of best fit 
    scale_color_manual(values =c("BEO-B05"="#6BAED6", "BEO-B06"= "#084594"))
  
  #"TNHA-BASE" = "#1B9E77","TNHA-SA" = "#D95F02", "TNHA-SB"="#7570B3", "TNHA-SC" = "#E7298A", "TNHA-SD"="#66A61E", "BEO-B05"="#6BAED6", "BEO-B06"= "#084594"
...
linear regression of XY plot above
```{r}
#adjust the XY_grnd_grnd filter in the plotting chunk above to adjust which sensors you're comparing
fit2 <- lm(grndtmp ~ grndtmp, data = XY_grnd_vs_grnd)
summary(fit2)
```