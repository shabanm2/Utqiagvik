...
```{r}
#grndtmp_3.5cm <- grndtmp %>% filter(depth == "10cm")
...
for(sensorName in sensorList){
  #Make sure sensor has data - otherwise will throw error
    ...
    daily = grndtmp_3.5cm %>% filter(sensor==sensorName) %>% filter(as.Date(Date, "%Y-%m-%d") <= datemax) %>%
...
if(nrow(daily) > 0){
  ...
  daily = data.frame(daily, station, sensor)
  avgdailyground_TNHA = rbind(avgdailyground_TNHA, daily)
}
...
Aggregated2 = grndamp %>% distinct(sensor, Maxval,Minval, mean_val, amplitude) #over whole smummer season, just the average over the whole season per sensor
...
grndamp$airavg <- averages$airtemp
amp_figs = grndamp %>% filter(!sensor == "TNHA-SB", !sensor == "TNHA-SD", !sensor == "TNHA-SA") 
  ggplot(data = amp_figs, aes(x=airtemp, y = grndtmp, color=sensor)) +
  geom_point()+
  theme_bw() + 
    labs(title= "TNHA-BASE vs. TNHA-SC, 6/19/22-9/19/22", x = "Air Temperature", y="Ground Temperature Amplitude",color="Station") + 
  theme(axis.text.y = element_text(size = 15)) +
  theme(axis.text.x=element_text(angle = 45, hjust = 1.0, size = 15), 
        axis.line = element_line(colour = "black", size =0.5, 
                                 linetype = "solid")) +
