...
solarrad$variable_3 <- NULL # you need to check the dataframe BEFORE running this line to see if this is the row you want to remove -- different on every computer
...
```{r fig8, fig.height = 12, fig.width = 18}
ggplot() +
  geom_point(data=averages, aes(x=grndtmp, y=solar, color = sensor), size = 3) +

  theme_bw() +
  labs(title= "Ground Temperature v. Solar Radiation", x="Ground Temperature (°C)", y="Solar Radiation (W/"~m^2~")",color="Station") + 
  #scale_x_datetime(breaks = datestep, minor_breaks = dateminstep) +
  
  
  theme(axis.text.y =element_text(face="bold",size=16,family="Times New Roman") ,axis.text.x=element_text(face = "bold",size = 16,family="Times New Roman"), axis.line = element_line(colour = "black", size =0.5, linetype = "solid")) +
